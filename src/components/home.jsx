...
export default function HomePage() {
  ...
  const getDoctorName = (doctor) => {
    return doctor.fullname || doctor.fullName || doctor.name || "";
  };

  const handleSpecializationSelect = (special) => {
    const title = typeof special === "string" ? special : special.title;

    navigate(
      `/FindDoctors?specialization=${encodeURIComponent(
        title
  ...
  const handleDoctorSelect = (doctor) => {
    ...
    navigate(
      `/FindDoctors?search=${encodeURIComponent(
        doctorName
      )}&location=${encodeURIComponent(locationValue)}`
    );
  };
  ...
  return (
    <div className="min-vh-100 d-flex flex-column">
      ...
      <main className="flex-grow-1">
        ...
        >
          {drinfo.map((dr, i) => (
            <Link key={i} className="text-decoration-none text-dark" to={dr.route}>
          ...
          <div className="d-flex gap-3 text-center justify-content-between">
            {healthConcern.map((drs, i) => (
              <div key={i} className="d-flex flex-column my-5">
                ...
                <Link
                  to={`/newconsultation?id=${drs.consultid}`}
        ...
        <section className="container my-5">
          ...
          <div className="position-relative">
            ...
            {swap < BookanAppoitment.length - 4 && (
              <button
                onClick={next}
            ...
            <div className="d-flex gap-4">
              ...
              <div>
                <img
                  ...
                  width={200}
                />
                <p style={{ width: "200px" }}>VITAMINS AND SUPPLEMENTS</p>
